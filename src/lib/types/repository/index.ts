  ...
  ? keyof TColumnsDecorator<M> extends never
    ...
    : Partial<{
        [K in
          ...
          | keyof TRelationsDecorator<M>]: K extends
          | `${string}.${string}`
          | TRawStringQuery
          ? boolean
...
> = [unknown, unknown] extends [T, R]
  ? keyof TColumnsDecorator<M> extends never
    ...
    : Partial<{
        [K in
          ...
          | `${string}.${string}`
          | TRawStringQuery]: K extends keyof TRelationsDecorator<M>
  ...
  : Partial<{
      [K in
        ...
        | TRelationKeys<R>
        | `${string}.${string}`
        | TRawStringQuery]: K extends TRelationKeys<R>
        ? K extends `$${string & K}`
          ? R extends Record<string, any>
            ? R[K]
            : never
...
export type TRepositoryRelation<R = unknown, M extends Model = Model> = [
  unknown,
] extends [R]
  ? keyof TRelationsDecorator<M> extends never
    ? Record<string, any>
    : Partial<{
        [K in keyof TRelationsDecorator<M>]: K extends keyof TRelationsDecorator<M>
          ? TRelationsDecorator<M>[K] extends (infer U)[]
            ? U extends Model
              ?
                  | boolean
...
> & {
  selectRaw?: SRS extends {}
    ? SRS & {
        [K in keyof SRS]: SRS[K] extends TRawStringQuery ? SRS[K] : never;
      }
...
export type TRepositoryPrimitiveExtendType<T> = T extends NumberConstructor
  ? number
    ...
    : T extends BooleanConstructor
      ...
      : T extends DateConstructor
        ...
        : T extends readonly (infer U)[]
          ? TRepositoryPrimitiveExtendType<U>[]
