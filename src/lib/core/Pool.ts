...
import { MariadbDriver }    from './Driver/mariadb';
import { MssqlDriver }      from './Driver/mssql';
import { Sqlite3Driver }    from './Driver/sqlite3';
...
export class PoolConnection extends EventEmitter {
    ...
    private _driver() {
        return String(this.OPTIONS.get('driver') ?? 'mysql2') as 'pg' | 'postgres' | 'mysql' | 'mysql2' | 'mariadb' | 'mssql' | 'sqlite3'
    }
}
