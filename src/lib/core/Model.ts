...
> extends AbstractModel<TS, TR>  {
     ...
     */
    static formatPattern<T extends Record<string, any> | string>({ data , pattern } : { data : T, pattern : TPattern }) : T {
        ...
        if(pattern === 'snake_case') {
            ...
            if(typeof data === 'string') {
                return data.replace(/([A-Z])/g, (str : string) => `_${str.toLocaleLowerCase()}`) as T
            }
        ...
        if(pattern === 'camelCase') {
            if(typeof data === 'string') {
                return data.replace(/(.(_|-|\s)+.)/g, (str : string) => str[0] + (str[str.length-1].toUpperCase()))as T
            }
    ...
    protected globalScope <T extends Model>(callback : (query : T) => T) : this {
        ...
        if(repository instanceof Promise) throw new Error('"whereQuery" is not supported a Promise')

        if(!(repository instanceof Model)) throw new Error(`Unknown callback query: '${repository}'`)
    ...
    protected useLoadRelationsInRegistry (): this {
        ...
        const relations : string[]  = this.$state.get('RELATION').map((r: { name: string })=> String(r.name))

        if(relations.length) this.relations(...Array.from(new Set(relations)) as any[])
    ...
    protected usePattern (pattern :  "snake_case" | "camelCase") : this {
        ...
        if(!allowPattern.includes(pattern)) {
            throw this._assertError(
                `The 'tspace-mysql' support only pattern '${this.$constants('PATTERN').snake_case}', 
                '${this.$constants('PATTERN').camelCase}'`
            )
        }
     ...
     * }
     */
    protected useTimestamp (timestampFormat ?: { createdAt : string , updatedAt : string }): this {
    ...
    */
    protected useHooks (arrayFunctions : Function[]): this {
        ...
        for(const func of arrayFunctions) {
            if(typeof func !== "function") throw this._assertError(`this 'function' is not a function`)
    ...
    */
    ...
    protected async exceptColumns (): Promise<string[]>  {
        ...
        const names = excepts.map((except:string) => {
    ...
    protected buildMethodRelation<K extends TR extends object ? keyof TR : string>(name: K , callback?: Function): this {
        ...
        const relation : TRelationOptions<K> = this.$state.get('RELATIONS').find((v: { name: string }) => v.name === name)
    ...
    exceptTimestamp (): this {
        ...
        if(this.$state.get('SOFT_DELETE')) {
            const deletedAt: string = this._valuePattern(this.$state.get('SOFT_DELETE_FORMAT'))
            excepts = [
                ...excepts,
                deletedAt
            ]
        ...
        const updatedAt: string = this._valuePattern(this.$state.get('TIMESTAMP_FORMAT').UPDATED_AT)
        const createdAt: string = this._valuePattern(this.$state.get('TIMESTAMP_FORMAT').CREATED_AT)
    ...
    async makeSelectStatement() : Promise<string> {
        ...
        const columns = Object.keys(schemaModel).map((column:string) => `\`${this.getTableName()}\`.\`${column}\``)
    ...
    async makeInsertStatement() : Promise<string> {
        ...
        const columns = Object.keys(schemaModel).map((column:string) => this.bindColumn(column))
    ...
    copyModel (instance : Model , options ?: { 
        ...
        if(options?.relations  == null || !options.relations) newInstance.$state.set('RELATIONS',[])
        if(options?.insert  == null || !options.insert) newInstance.$state.set('INSERT','') 
        if(options?.update  == null || !options.update) newInstance.$state.set('UPDATE','')
        if(options?.delete  == null || !options.delete) newInstance.$state.set('DELETE','')
        if(options?.select  == null || !options.select) newInstance.$state.set('SELECT',[])
        if(options?.join    == null || !options.join) newInstance.$state.set('JOIN',[])
        if(options?.where   == null || !options.where) newInstance.$state.set('WHERE',[])
        if(options?.groupBy == null || !options.groupBy) newInstance.$state.set('GROUP_BY',[])
        if(options?.having  == null || !options.having) newInstance.$state.set('HAVING','')
        if(options?.orderBy == null || !options.orderBy) newInstance.$state.set('ORDER_BY',[])
        if(options?.limit   == null || !options.limit) newInstance.$state.set('LIMIT','')
        if(options?.offset  == null || !options.offset) newInstance.$state.set('OFFSET','')
     ...
     * @returns {this} this
     */
    protected async _queryStatement (sql: string , { retry = false } = {}) : Promise<any[]> {
        try {
            ...
            const logger = async (results : any[]) => {
                ...
                const loggerOptions : { selected : boolean } = this.$state.get('LOGGER_OPTIONS')
    ...
    protected async _actionStatement ({ sql  , returnId = false} : { sql : string, returnId?: boolean }) : Promise<any> {
        try {
            ...
            await this._checkSchemaOrNextError(error,Number(this.$state.get('RETRY')))
    ...
    * The 'ignoreSoftDelete' method is used to disable the soft delete.
    ...
    withNotExists<K extends TR extends object ? keyof TR : string>(...nameRelations: K[]): this {
        ...
        this.$state.set('RELATIONS',this.$relation.apply(nameRelations,'notExists'))
    ...
    protected hasOneBuilder({ 
        ...
        foreignKey, 
        freezeTable 
    } : TRelationQueryOptions, callback ?: Function): this {
    ...
    onlyTrashed(): this {
        ...
        const column = this._valuePattern(
    ...
    whereNotIn<K extends keyof TS | `${string}.${string}` | TRawStringQuery | TFreezeStringQuery>(column: K , array : any[]): this {
        ...
        const values = `${array.map((value:string) => this._checkValueHasRaw(this.$utils.escape(value))).join(',')}`
    ...
    whereQuery<T extends Model>(callback : (query : T) => T) : this {
        ...
        if(!(repository instanceof Model)) throw this._assertError(`Unknown callback query: '${repository}'`)
    ...
    */
    async first<K>(cb ?: Function): Promise<TS & K & Partial<TR extends any ? TS & Partial<TR> : TR> | null> {
        ...
        if(this.$state.get('RELATIONS_EXISTS')) sql = String(this.$relation.loadExists())  
    ...
    */
    ...
    updateMultiple<K extends keyof TS | TRawStringQuery | TFreezeStringQuery>(
        ...
        const updateColumns : Record<string,any> = cases.reduce((columns : Record<string,any[]>, item) => {
        ...
        const columns : Record<string,any> = cases.reduce((columns : Record<string,string>, item) => {
        ...
        for(let i = cases.length - 1; i >= 0; i--) {
            const c = cases[i] as  unknown as { when : Record<string,any> , columns : Record<string,any>}
            ...
            if(this.$state.get('TIMESTAMP')) {
                const updatedAt: string = this._valuePattern(this.$state.get('TIMESTAMP_FORMAT').UPDATED_AT)
                c.columns[updatedAt]= c.columns[updatedAt] === undefined ? this.$utils.timestamp() : this.$utils.covertDateToDateString(c.columns[updatedAt])
    ...
    async save (): Promise<Record<string,any> | any[] | null | undefined> {
        ...
        switch (String(this.$state.get('SAVE'))) {
            ...
            case 'INSERT_OR_SELECT' : return await this._insertOrSelectModel()
     ...
     */
    async faker<T extends TS>(rows: number, callback ?: (results : T , index : number) => T) : Promise<void> {
        ...
        const fields : Array<{ Field: string , Type: string }> = schemaModel == null
        ...
        const passed = (field : string) => ['id','_id',deletedAt].some(p => field === p)
     ...
     */
     async sync ({ force = false, foreign = false , changed = false } = {}): Promise<void> {
        ...
        const existsTables = checkTables.map((c: { [s: string]: unknown } | ArrayLike<unknown>) => Object.values(c)[0]) as string[]

        const schemaModel = this.getSchemaModel()

        if(schemaModel == null) throw this._assertError('The schema model does not exist.')

        const checkTableIsExists = existsTables.some((table: string) => table === this.getTableName())
    ...
    private async _checkTableLoggerIsExists () {
        ...
        const checkTables = await new DB().rawQuery(`${this.$constants('SHOW_TABLES')} ${this.$constants('LIKE')} '${tableLogger}'`)
    ...
    private _classToTableName (className?: string | null, { singular = false } = {}) {

        if(className == null) className = this.constructor.name
      
        const tb = className.replace(/([A-Z])/g, ( str:string ) => '_' + str.toLowerCase()).slice(1)
    ...
    private _showOnly  (data : any) {
        ...
        let result: any[] = []
        const hasNameRelation = this.$state.get('RELATIONS').map((w :any) => w.as ?? w.name)
    ...
    private async _validateSchema(data : Record<any,string> , action : 'insert' | 'update') {
        ...
        const schemaTableDefined = this.$state.get('VALIDATE_SCHEMA_DEFINED') as null

        const schema = schemaTableDefined ?? Object.keys(schemaTable).reduce((acc : Record<string,any>, key) => {
        ...
        for(const column in schema) {
            ...
            if(s.max && r > s.max) throw this._assertError(`This column "${column}" is more than "${s.max}"`)

            if(s.min && r < s.min) throw this._assertError(`This column "${column}" is less than "${s.min}"`) 

            if((s.enum && s.enum.length) && !s.enum.some((e: string | number) => e === r)) {
                throw this._assertError(`This column "${column}" is must be in ${s.enum.map((e: string| number) => `"${e}"`)}`) 
            }
            ...
            if(s.unique && action === 'insert') {
                ...
                if(exist) throw this._assertError(`This column "${column}" is duplicated`)
            }
        }
    ...
    private async _execute ({ sql , type , message , options } : TExecute) {
        ...
        if(!result.length) return this._returnEmpty(type, result, message , options)
        ...
        return await this._returnResult(type, result) || this._returnEmpty(type, result, message , options)
    }
    ...
    private async _returnEmpty  (type:string , result:any[], message ?: string , options ?: Record<string,any>) {
        ...
        switch(type) {
            ...
            case 'GET' : {
                ...
                if(this.$state.get('CHUNK')) {
                    const r = data.reduce((resultArray: any[][], item: any, index: number) => { 
    ...
    private async _attach (name: string , dataId :any[] , fields ?: any) {
        ...
        const relation: TRelationOptions = this.$state.get('RELATION')?.find((data: any) => data.name === name)
        ...
        try {
            ...
            const success =  await new DB().table(pivotTable).createMultiple(
    ...
    private async _detach (name: string , dataId :any[])  {
        ...
        const relation :any = this.$state.get('RELATION').find((data: any) => data.name === name)
    ...
    private _queryUpdateModel(objects : Record<string,any>) {
        ...
        if(this.$state.get('TIMESTAMP')) {
            const updatedAt: string = this._valuePattern(this.$state.get('TIMESTAMP_FORMAT').UPDATED_AT)
            objects =  { ...objects, 
                [updatedAt] :  objects[updatedAt] === undefined ? this.$utils.timestamp() : this.$utils.covertDateToDateString(objects[updatedAt]),
            }
        }
    ...
    private _queryInsertModel (data: Record<string,any>) {
        ...
        if(hasTimestamp) {
            ...
            data = { 
                ...data ,
                [createdAt] : data[createdAt] === undefined ? this.$utils.timestamp() : this.$utils.covertDateToDateString(data[createdAt]),
                [updatedAt] : data[updatedAt] === undefined ? this.$utils.timestamp() : this.$utils.covertDateToDateString(data[updatedAt])
            }
        }
        ...
        const columns: string[] = Object.keys(data).map((column:string) => this.bindColumn(column))
    ...
    private _queryInsertMultipleModel (data : any[]) {
        ...
        for(let objects of data) {
            ...
            if(hasTimestamp) {
                ...
                objects =  { 
                    ...objects , 
                    [createdAt] : objects[createdAt] === undefined 
                        ? this.$utils.timestamp() 
                        : this.$utils.covertDateToDateString(objects[createdAt]),
                    [updatedAt] : objects[updatedAt] === undefined 
                        ? this.$utils.timestamp() 
    ...
    private async _insertOrSelectModel () : Promise<{[key :string] :any} | {[key :string] :any}[] | null> {
    ...
    private _assertError(condition : boolean | string = true , message : string = 'error') {
    ...
    private _validateMethod (method : string) : void {
        ...
        switch (method.toLocaleLowerCase()) {
            ...
            case 'get'           : {
                ...
                const findMethodNotAllowed = methodCallings.find((methodCalling : string) => methodsNotAllowed.includes(methodCalling))

                if(methodCallings.some((methodCalling : string) => methodsNotAllowed.includes(methodCalling))) {
            ...
            case 'save' : {
                ...
                if(!methodCallings.some((methodCalling : string) => methodsSomeAllowed.includes(methodCalling))) {
    ...
    private async _checkSchemaOrNextError (e: unknown, retry = 1) : Promise<void> {
        try {
            ...
            if(!errorMessage.toLocaleLowerCase().includes("doesn't exist")) return this._stoppedRetry(e)
    ...
    private async _observer (result : unknown, type : 'selected'| 'created'| 'updated' | 'deleted') {
