...
class Builder extends AbstractBuilder {
    ...
    unset (options : { 
        ...
        if(options?.groupBy != null && options.groupBy) this.$state.set('GROUP_BY',[])
        if(options?.having  != null && options.having) this.$state.set('HAVING','')
    ...
    selectArray (object : Record<string,`${string}.${string}`> , alias : string): this {
        ...
        if(!Object.keys(object).length) throw new Error("The method 'selectArray' is not supported for empty object")
     ...
     * @returns   {this}
     */
    whereJSON (column : string , { key, value , operator } : { key : string, value : string , operator ?:string }): this {
    ...
    whereQuery(callback : Function) : this {
        ...
        if(!(repository instanceof DB)) throw new Error(`Unknown callback query: '${repository}'`)
     ...
     * @returns {this}
     */
    orWhereCases (cases: { when : string , then : string }[]  , elseCase ?: string): this {
    ...
    joinSubQuery ({
        ...
        foreignKey,
        sql
    } : { localKey : string; foreignKey : string , sql : string}): this {
     ...
     * @returns {this}
     */
    orderBy (column: string , order : 'ASC' | 'asc' | 'DESC' | 'desc' = 'ASC'): this {
    ...
    latestRaw (...columns: string[]): this {
        ...
        if(columns?.length) {
            orderBy = columns.map(column => {
                if(column.includes(this.$constants('RAW'))) return column?.replace(this.$constants('RAW'),'')
                return column
            }).join(', ')
        }
    ...
    update (data : Record<string,any> , updateNotExists : string[] = []): this {
        ...
        if(!Object.keys(data).length) throw new Error('This method must be required')
     ...
     * @returns {this} this
     */
    updateMultiple(cases: { when : Record<string,any> , columns : Record<string,any> }[]): this {

        if(!cases.length) throw new Error(`The method 'updateMultiple' must not be empty.`)
        ...
        for(let i = cases.length - 1; i >= 0; i--) {
            ...
            if(c.when == null || !Object.keys(c.when).length) throw new Error(`This 'when' property is missing some properties`)
            if(c.columns == null || !Object.keys(c.columns).length) throw new Error(`This 'columns' property is missing some properties`)
    ...
    */
    hook (func : Function): this {
        if(typeof func !== "function") throw new Error(`this '${func}' is not a function`)
    ...
    */
    ...
    async version () : Promise<string> {
        const result =  await this._queryStatement(`${this.$constants('SELECT')} VERSION() as version`)
     ...
     * @returns {promise<Pagination>}
     */
    async paginate(paginationOptions ?: { limit ?: number , page ?: number }) : Promise<TPagination> {
        let limit = 15
        let page = 1
    ...
    async first (cb ?: Function): Promise<Record<string,any> | null> {
        ...
        if(cb) {
            ...
            if(callbackSql == null || callbackSql === '') throw new Error('Please provide a callback for execution')
            sql = callbackSql
        }
        ...
        if(this.$state.get('PLUCK')) {
            ...
            if(!checkProperty) throw new Error(`can't find property '${pluck}' of result`)
     ...
     */
    async firstOrError(message : string, options ?: Record<string,any> ): Promise<Record<string,any>>{
     ...
     * @returns {promise<object | null>}
     */
    async findOneOrError(message : string, options ?: Record<string,any> ): Promise<Record<string,any>>{
    ...
    async get (cb ?: Function): Promise<any[]> {
        ...
        if(this.$state.get('CHUNK')) {
            const data = result.reduce((resultArray: any[][], item: any, index: number) => { 
    ...
    async avg (column: string ='id'): Promise<number> {
        ...
        return Number(this._resultHandler((result.reduce((prev , cur) => prev + Number(cur?.aggregate ?? 0),0) || 0) / result.length))
    }
     ...
     * @returns {Promise<Array>}
     */
    async showColumns (table : string = this.$state.get('TABLE_NAME')) : Promise<string[]> {
        const sql : string = [
            `${this.$constants('SHOW')}`,
            `${this.$constants('COLUMNS')}`,
            `${this.$constants('FROM')}`,
     ...
     * @returns {Promise<Array>}
     */
    async showSchema (table : string = this.$state.get('TABLE_NAME')) : Promise<string[]>{
     ...
     * @returns {Promise<Array>}
     */
    async showSchemas (table : string = this.$state.get('TABLE_NAME')) : Promise<string[]>{
     ...
     * @returns {Promise<Array>}
     */
    async showValues (table : string = this.$state.get('TABLE_NAME')) : Promise<string[]> {
        const sql : string = [
            `${this.$constants('SELECT')}`,
            '*',
            `${this.$constants('FROM')}`,
     ...
     * @returns {promise<any>}
     */
    async faker(rows: number, cb ?: (results : Record<string,any> , index : number) => Record<string,any>) : Promise<void> {
        ...
        const fields : Array<{ Field: string , Type: string }> = await this._queryStatement(sql)
     ...
     * @returns {promise<boolean>}
     */
    async truncate ({ force = false } : {force ?: boolean } = {}): Promise<boolean> {
    ...
    protected copyBuilder (instance : Builder , options ?: { 
        ...
        if(!(instance instanceof Builder)) throw new Error('Value is not a instanceof Builder')
    ...
    protected async _actionStatement ({ sql  , returnId = false} : { sql : string, returnId?: boolean }) {
    ...
    private async _insertNotExists ()  {
        ...
        if(!this.$state.get('WHERE').length) throw new Error("Can't insert not exists without where condition")
    ...
    private async _update (ignoreWhere = false) {
        ...
        if(!this.$state.get('WHERE').length && !ignoreWhere) throw new Error("can't update without where condition")
    ...
    private _queryInsertMultiple (data : any[]) {
        ...
        const columns: string[] = Object.keys([...data]?.shift()).map((column: string) => this.bindColumn(column))
    ...
    private _initialConnection () {
        ...
        this.$constants = (name) => {
            ...
            if(!CONSTANTS.hasOwnProperty(name)) throw new Error(`Not found that constant : '${name}'`)
