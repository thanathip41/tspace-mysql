...
class DB extends AbstractDB {
     ...
     * @returns {promise<any[]>}
     */
    static async query (sql: string , parameters : Record<string,any> = {}) : Promise<any[]> {
    ...
    jsonObject(object : Record<string,string> , alias : string): string {
        ...
        if(!Object.keys(object).length) throw new Error("The method 'jsonObject' is not supported for empty object")
     ...
     * @returns {string} string
     */
    caseUpdate (cases: { when : string , then : string }[]  , final ?: string): string | [] {
     ...
     * @returns {this}
     */
    static caseUpdate (cases: { when : string , then : string }[] , final ?: string): string | [] {
     ...
     * @returns {Connection}
     */
    static async getConnection(options : TConnectionOptions) : Promise<TConnection> {
    ...
    removeProperties(data: any[] | Record<string,any>, propertiesToRemoves: string[]): Array<any> | Record<string,any> {
        ...
        const remove = (obj: Record<string,any>, propertiesToRemoves: string[]) => {
    ...
    async cloneDB (database : string): Promise<void> {
        ...
        if(Object.values(db[0]?? []).length) throw new Error(`This database : '${database}' is already exists`)
        ...
        const creating = async ({ table , values } : { table : string , values ?: string }) => {
           try {
    ...
    async backup ({ database, to } : TBackup): Promise<void> {
        ...
        if(to != null && Object.keys(to).length) this.connection({ ...to , database })
    ...
    static async backupToFile({ filePath, database , connection } : TBackupToFile ): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    static async backupSchemaToFile ({ filePath, database , connection } : TBackupToFile ): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    static async backupTableToFile({ filePath, table, connection } : TBackupTableToFile ): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    static async backupTableSchemaToFile({ filePath, table, connection } : TBackupTableToFile ): Promise<void> {
        return new this().backupTableSchemaToFile({ filePath, table, connection})
    }

    private async _backup ({tables , database } : {tables : string[] , database : string }) {
