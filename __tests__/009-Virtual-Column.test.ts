...
describe("Testing Virtual Column", function () {
  /* ##################################################### */
  it(`Virtual Column: Start to get virtual column 'email_username' from Model User`, async function () {
    const user = await new User().first();

    expect(user).to.be.an("object");

    expect(user?.email_username).to.be.equal(
    ...
    expect(users).to.be.jsonSchema({
      type: "array",
      items: { ...virtualJsonSchema },
    });
  ...
  it(`Virtual Column: use method .select('email','username','email_username')`, async function () {
    const user = await new User()
      .select("email", "username", "email_username")
      .first();
  ...
  it(`Virtual Column: use method .where('email_username')`, async function () {
    const userNull = await new User()
      .where("email_username", "LIKE", "%xxxx%")
      .first();

    expect(userNull).to.be.equal(null);

    const user = await new User()
      .where("email_username", "LIKE", "%test01%")
      .first();
